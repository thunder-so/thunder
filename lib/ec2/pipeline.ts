  ...
  private createBuildProject(props: Ec2Props): PipelineProject {
    ...
    if (props.buildProps?.buildSystem === "Nixpacks") {
      const installCmd = props.buildProps?.installcmd
        ...
        : "";
      const buildCmd = props.buildProps?.buildcmd
        ? `--build-cmd "${props.buildProps.buildcmd}"`
        : "";
      const startCmd = props.buildProps?.startcmd
        ? `--start-cmd "${props.buildProps.startcmd}"`
        : "";
    ...
    const buildSpec = BuildSpec.fromObject({
      ...
      phases: {
        ...
        build: {
          commands: buildCommands.slice(
            buildCommands.indexOf(
              `docker build -t $ECR_REPO:$IMAGE_TAG -f ${dockerfilePath} .`,
            ),
            buildCommands.indexOf("docker push $ECR_REPO:$IMAGE_TAG") + 1,
          ),
        },
        post_build: {
      ...
      artifacts: {
        files: this.rootDir
          ...
          : ["imageUri.txt", "imageTag.txt", "imageDigest.txt"],
      },
    });
