  ...
  private setupSyncAction(props: PipelineProps): Project {
    ...
    syncActionRole.addToPolicy(
      new PolicyStatement({
        ...
        resources: [
          `arn:aws:cloudfront::${props.Distribution.stack.account}:distribution/${props.Distribution.distributionId}`,
        ],
      }),
    );
  ...
  private createBuildProject(props: PipelineProps): Project {
    ...
    if (props.buildSpecFilePath) {
      ...
      try {
        ...
        throw new Error(
          `Failed to read build spec file at ${props.buildSpecFilePath}: ${error}`,
        );
      }
