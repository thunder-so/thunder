...
export function generateNixpacksDockerfile(
  ...
  if (!fs.existsSync(absRootDir)) {
    throw new Error(
      `[nixpacks] Source directory does not exist: ${absRootDir}`,
    );
  }

  const installCmd = buildProps?.installcmd
    ? `--install-cmd "${buildProps.installcmd}"`
    : "";
  const buildCmd = buildProps?.buildcmd
    ? `--build-cmd "${buildProps.buildcmd}"`
    : "";
  const startCmd = buildProps?.startcmd
    ? `--start-cmd "${buildProps.startcmd}"`
    : "";
