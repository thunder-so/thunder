...
export class ServiceConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: FargateProps) {
    ...
    const taskDef = new TaskDefinition(this, "Task", {
      ...
      runtimePlatform: {
        cpuArchitecture:
          props.serviceProps?.architecture ?? CpuArchitecture.X86_64,
      },
    });
    ...
    const container = taskDef.addContainer("Container", {
      ...
      environment: {
        HOSTNAME: "0.0.0.0",
        ...(props.serviceProps?.variables?.reduce(
          (acc, obj) => ({ ...acc, ...obj }),
          {},
        ) ?? {}),
      },
        ...
        ? (Object.fromEntries(
            props.serviceProps.secrets.map((s) => [
              ...
              Secret.fromSecretsManager(
                SecretsManagerSecret.fromSecretAttributes(
                  this,
                  `${s.key}Secret`,
                  {
    ...
    container.addPortMappings({
      containerPort: props.serviceProps?.port || 3000,
